...
impl From<JsonValue> for FileInfo {
	fn from(value: JsonValue) -> Self {
		...
		let additions = file_info.get("additions").and_then(|v| v.as_u64()).unwrap_or(0);
		let deletions = file_info.get("deletions").and_then(|v| v.as_u64()).unwrap_or(0);
		Self {
			file_name: file_info
				.get("filename")
